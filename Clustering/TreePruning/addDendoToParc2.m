...
function [dendo_mat,node_list,self_cons] = addDendoToParc2(sub_id,parc_sc)


% Load Prune Struct
prune_struct = load('avgsc_prune_struct.mat');

parc_size = size(parc_sc,1); 

[~,~,term_nodes] = fast_term_to_clusters(prune_struct.treeSeq,prune_struct.node_mat,...
    find(prune_struct.lengthSeq <= parc_size,1),prune_struct.link);


% Load High Resolution SC Matrix for Subject 
full_sc = load('SUB_'+string(sub_id)+'SC_MAT.m'); 
